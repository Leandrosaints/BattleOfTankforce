...
item_images = {
    ...
    'shield': pygame.image.load("img/itens/escudo.png").convert_alpha(),
    'upgrade': pygame.image.load("img/itens/misseis.png").convert_alpha(),
    'helicopter_call': pygame.image.load("img/itens/helicopter.png").convert_alpha(),  # Imagem do power-up
}
...
class Helicopter:
    def __init__(self, tank_pos):
        self.x = tank_pos[0]
        self.y = tank_pos[1] - 50  # Posição acima do tanque
        self.img = item_images['helicopter_call']  # Imagem do helicóptero
        self.bombs = []  # Lista de bombas que o helicóptero soltará
        self.drop_timer = 30  # Tempo entre as bombas

    def update(self):
        # Atualiza a posição do helicóptero para seguir o tanque
        self.x = tank_pos[0]
        self.y = tank_pos[1] - 50
        
        # Lógica para soltar bombas
        if self.drop_timer <= 0:
            self.bombs.append(Bomb((self.x, self.y + 20)))  # Soltar bomba
            self.drop_timer = 30  # Reinicia o timer
        else:
            self.drop_timer -= 1
...
class Bomb:
    def __init__(self, pos):
        self.x, self.y = pos
        self.speed = 5  # Velocidade da bomba
        self.size = 10  # Tamanho da bomba

    def update(self):
        self.y += self.speed  # A bomba cai
...
def main():
    global game_over, shield_active, shield_timer, tank_health, tank_speed, helicopter_active, helicopter
    ...
    while not game_over:
        ...
        for item in items[:]:
            ...
            if item.get_rect().colliderect(pygame.Rect(tank_pos[0], tank_pos[1], tank_width, tank_height)):
                if item.type == 'health':
                    ...
                    tank_speed += 1
                elif item.type == 'helicopter_call':
                    helicopter_active = True
                    helicopter = Helicopter(tank_pos)
                items.remove(item)
        ...
        if helicopter_active:
            ...
            # Verifica se as bombas colidem com inimigos
            for bomb in helicopter.bombs[:]:
                bomb.update()
                if bomb.y > HEIGHT:  # Remove bombas que saem da tela
                    helicopter.bombs.remove(bomb)
                for enemy in enemies[:]:
                    if bomb.get_rect().colliderect(enemy.get_rect()):
                        enemy.health -= 1
                        if enemy.health <= 0:
                            enemies.remove(enemy)
                        helicopter.bombs.remove(bomb)
